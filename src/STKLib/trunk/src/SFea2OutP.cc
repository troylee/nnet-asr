...
#define SVN_DATE       "$Date: 2007-07-11 21:51:58 +0800 (Wed, 11 Jul 2007) $"
#define SVN_AUTHOR     "$Author: glembek $"
#define SVN_REVISION   "$Revision: 195 $"
#define SVN_ID         "$Id: SFea2OutP.cc 195 2007-07-11 13:51:58Z glembek $"
...
////      case 'x': hmm_ext      = optarg; break;
  ...
  for (file_name = feature_files; file_name; file_name = file_name->mpNext) {
    ...
    ReadHTKFeatures(file_name->mpPhysical, swap_features,
                    startFrmExt, endFrmExt, targetKind,
                    derivOrder, derivWinLengths, &header, feature_matrix);
#endif
    ...
    {
      UpdateStacks(&hset, feature_matrix[i], ++time, FORWARD);
      ...
      for (m = 0; m < hset.mHmmHash.mNEntries; m++) 
      {
        ...
        for (j = 0; j < ((Hmm *) macro->mpData)->mNStates-2; j++) {
          ...
          if (!state->mpMacro) { // take only non-shared states
            opp[state->mID] =
              DiagCGaussianMixtureDensity(state, feature_matrix[i], NULL);
          }
        }
      }
